...
NO_ADMIN = "`Kamu Bukan Admin!!`"
NO_PERM = "`Kamu Tidak Mempunyai Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Kamu Bukan Admin,`"
...
async def promote(promt):
    ...
    try:
        ...
        await promt.edit("`Berhasil Promote!`")
        await sleep(5)
        await promt.delete()
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` Berhasil Di Banned !!\n`Karena:` {reason}"
        )
    else:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` Berhasil Di Banned !"
...
async def gspider(gspdr):
    ...
    await gspdr.edit("`Berhasil Dibisukan Pengguna!`")
    if gmute(user.id) is False:
        ...
        if reason:
            ...
            await gspdr.edit("`Berhasil Dibisukan Pengguna Secara Global!`")
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`Mencari Akun Mati/terhapus...`")
        ...
        if del_u > 0:
            del_status = (
                f"`Menemukan` **{del_u}** `Akun Mati/terhapus Di Dalam Grup Ini,"
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit("Grup Ini Terlalu Rame Mengunggah Daftar Pengguna Sebagai File.")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Grup Ini Terlalu Rame, Mengunggah Daftar Akun Terhapus Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
